...
| `WHATSAPP_WORKER_SEND_URL` (the worker's `/send` route; a bare origin is accepted and `/send` appended) + `INTERNAL_SEND_TOKEN` | no WhatsApp. A conserje with an email gets the check-in notice by email. A cleaner gets the cleaning confirmation by email instead                                                                                                                                                                                                                                                                                                                                                            |
