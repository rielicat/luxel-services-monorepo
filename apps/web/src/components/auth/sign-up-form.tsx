...
export function SignUpForm() {
  ...
  const onForm = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const onCode = async (e: React.FormEvent) => {
    ...
    setNotice(null);
    try {
      const done = await signUp.attemptEmailAddressVerification({ code: code.trim() });
    ...
    <div className="grid gap-4">
      {step === 'form' ? (
        <>
          ...
          <form onSubmit={onForm} noValidate className="grid gap-3">
