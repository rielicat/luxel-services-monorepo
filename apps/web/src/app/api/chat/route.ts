...
export async function POST(req: Request) {
  ...
  if (handoff) {
    const reply = 'Listo — un humano de Servicios Luxel te contactará por WhatsApp en breve.';
    await supabase.from('messages').insert({
      customer_id: customerId,
      session_id: sessionId,
      direction: 'out',
  ...
  const reply = match
    ...
    : {
        i18nKey: null,
        text: 'No encontré una respuesta automática. ¿Quieres hablar con una persona por WhatsApp?',
      };
