/**
 * End-to-end proof of the cleaning coordination slice: a check-out (imported
 * calendar block) yields a suggested cleaning priced by @luxel/pricing, which the
 * host can schedule; a re-run is idempotent and preserves host decisions.
 */
...
// unstable_cache must pass through — pricing-data.ts wraps getPricingData in it.
...
describe.skipIf(!LIVE)('cleaning coordination (end to end)', () => {
  it('suggests a cleaning from a check-out, schedules it, and stays idempotent', async () => {
    ...
    // Seed an imported check-out (as calendar sync would).
    ...
    // Re-running must not duplicate and must preserve the scheduled decision.
