...
export default async function TelemetryPage({
  ...
  return (
    <div>
      ...
      <DataTable head={['Evento', 'Ruta', 'Sesión', 'Origen', 'Propiedades', 'Hora']}>
        {events.map((e) => (
          <tr key={e.id} className="border-border/60 hover:bg-muted/40 border-b">
            ...
            <td className="text-muted-foreground px-4 py-3 font-mono text-xs">{e.path ?? '—'}</td>
            ...
            <td className="text-muted-foreground px-4 py-3 text-xs">{fmtDateTime(e.created_at)}</td>
          </tr>
        ))}
        {!events.length && <EmptyRow span={6}>Sin eventos en este rango.</EmptyRow>}
      </DataTable>
    </div>
  );
}
